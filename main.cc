...
g++ main.cc -lboost_filesystem -lboost_system -o main
or
g++ -L /usr/include/boost main.cc -lboost_filesystem -lboost_system -o main
...
mapSIVec docWordDict(smatrix words) // -------------------------------- look into this
{
...
mapSD getIDFs(mapSIVec allDocsWords) // go through vocab, for each word, go through each doc, if word occurs numDocsWithWord++
{
    mapSD idfs;
...
//goes through training data and picks up all categories
void getCats()
{
    for (int i = 0; i < docs.size(); i++) // go through training set
    {
        bool check = true;
        for (int j = 0; j < cats.size(); j++) // go through categories already picked up
        {
            if (docs[i].cat == cats[j]) // then category has already been picked up, do not pick up again
            {
                check = false;
                break;
            }
        }
        if (check) // if category has not already been picked up
        {
            cats.push_back(docs[i].cat); // push it into svec cats
        }
    }
}
...
wVec naiveBayesTrain(mapSDVec TFIDFvec) // complement naive bayes; words of more importance in one cat will have lower weight
{
    double currWeight = 0;
    wVec weights;
...
{
    // @TODO: use accumulator to speed up
    bool foundit = false;
    mapSD::iterator curr, endofmap;
    for (curr = catsScores.begin(), endofmap = catsScores.end(); curr != catsScores.end(); curr++) // go through each category's score
    {
        ...
        for (currIn = catsScores.begin(), endofmapIn = catsScores.end(); currIn != catsScores.end(); currIn++) // compare each against each other category's score
        {
            ...
            else // if it is higher than any, just skipp it.
            {
                foundit = false;
                break;
            }
...
void naiveBayesClassifyMany(docVec &docsToTest, bool output = false)
{
    ...
    {
        resultsFile.open ("results.txt", fstream::out);
    }
    ...
    for (int i = 0; i < allDocsDict2.size(); i++)
    {
        ...
        if (output) // if output is wanted in the console / results file
        {
            cout << "Predicted Cat: " << bestCat << ", Doc: " << docsToTest[i].title << endl << endl;
        }
    }
    if (output) // if output is wanted in the console / results file
        resultsFile.close();
...
void populateDocVec(string myDir, int i = 0)
{
    ...
    BOOST_FOREACH(fs::path const &p, make_pair(it, eod))
    {
        if(is_regular_file(p))
        {
            ...
            ifstream currFile(("Mini-Corpus/" + currCat + "/" + myfilename).c_str());
            ...
            if (currFile.is_open())
            {
                ...
                else if (j < testSetSize && j > trainingSetSize) // next 20% for testing
                {
...
int main()
{
    ...
    // populateDocVec("/home/jahanzeb/C++ Stuff/Summative Stuff/DATA/Mini-Corpus/"); // change this to where mini corpus is stored with cpp file (in project folder)
    populateDocVec("Mini-Corpus"); // change this to where mini corpus is stored with cpp file (in project folder)
    float secsElapsed2 = (float)(clock() - startTime2)/CLOCKS_PER_SEC;
    cout << "Time for loading documents " << secsElapsed2 << endl;
    ...
    if (trainBool == "y")
    {
        mapSDVec TFIDFvec = prepAndFindTFIDFs(docs, true);

        //train data
        weights = naiveBayesTrain(TFIDFvec);
        normalizeWeights();
    }
    else
        getWeightsFromFile();
    ...
    ofstream resultsFile; // open file to print results to
    resultsFile.open ("results.txt", fstream::out); // this is just to clear file of old contents

    checkAccuracy(testDocs); // checks accuracy of test set
    checkAccuracy(cvDocs); // checks accuracy of cross validation set
